...
const ACTIVE_STATUSES = new Set([
  "heat", "heating", "cool", "cooling", "fan", "fan_only", "fan-on", "fanstart", "running"
]);

function toBoolActive(equipment_status: string | null, is_active: boolean | null): boolean {
...
function deriveMode(equipment_status: string | null, mode: string | null): string {
...
export async function stitchSessions(client: PoolClient, tailSeconds: number) {
  // Iterate devices
  const devices = await client.query(`SELECT device_key FROM devices`);
  for (const d of devices.rows) {
    await stitchDevice(client, d.device_key, tailSeconds);
  }
}

async function ensureDeviceState(client: PoolClient, device_key: string): Promise<DeviceStateRow> {
...
async function stitchDevice(client: PoolClient, device_key: string, tailSeconds: number) {
  // Load state
  let state = await ensureDeviceState(client, device_key);

  // Pull new events since last_event_ts (inclusive? use > to avoid dup)
  ...
  if (evs.rows.length === 0) {
    // Also close stale open session if tail has elapsed
...
async function maybeCloseStale(client: PoolClient, device_key: string, state: DeviceStateRow, tailSeconds: number) {
  if (!state.open_session_id) return;

  // If device is active, don't close
  ...
  const dur = Math.max(0, dayjs.utc(ended_at).diff(dayjs.utc(started_at), "second"));
  ...
  // Update cumulative hours_used_total for device_states (respect last_reset_ts)
  ...
  if (lastReset && dayjs.utc(started_at).isBefore(lastReset)) {
    // Session started before reset but ended after; conservative approach: count only post-reset portion
