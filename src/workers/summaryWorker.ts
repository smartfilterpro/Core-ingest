...
export async function runSummaryWorker(pool: Pool, options?: { fullHistory?: boolean; days?: number }) {
  ...
  const query = `
    ...
    all_device_dates AS (
      -- UNION of all dates from both runtime_sessions and equipment_events
      -- This ensures we create summaries for days with activity but no runtime (idle days)
      SELECT DISTINCT device_key, device_id, date FROM (
        SELECT device_key, device_id, date FROM equipment_runtime_daily
        UNION
        SELECT aed.device_key, d.device_id, aed.date
        FROM all_event_dates aed
        INNER JOIN devices d ON d.device_key = aed.device_key
        WHERE d.device_id IS NOT NULL
      ) combined
    ),
    daily AS (
      -- Use all_device_dates as the PRIMARY driver (UNION of runtime and event dates)
      -- LEFT JOIN to runtime and event data
      -- This ensures summaries are created for idle days (0 runtime but has events)
      SELECT
        add.device_id,
        add.date as date,
      ...
      FROM all_device_dates add
      LEFT JOIN equipment_runtime_daily erd ON erd.device_key = add.device_key AND erd.date = add.date
      LEFT JOIN all_event_dates aed ON aed.device_key = add.device_key AND aed.date = add.date
      LEFT JOIN thermostat_mode_daily tmd ON tmd.device_key = add.device_key AND tmd.date = add.date
      GROUP BY add.device_id, add.date, erd.runtime_seconds_total, erd.runtime_seconds_heat,
