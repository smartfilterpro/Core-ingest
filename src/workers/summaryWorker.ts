  ...
  const query = `
    WITH daily AS (
      SELECT
        ...
        SUM(CASE WHEN rs.mode IN ('heat') THEN COALESCE(rs.runtime_seconds, 0) ELSE 0 END)::INT AS runtime_seconds_mode_heat,
        SUM(CASE WHEN rs.mode IN ('cool') THEN COALESCE(rs.runtime_seconds, 0) ELSE 0 END)::INT AS runtime_seconds_mode_cool,
        SUM(CASE WHEN rs.mode IN ('auto', 'heat-cool') THEN COALESCE(rs.runtime_seconds, 0) ELSE 0 END)::INT AS runtime_seconds_mode_auto,
        SUM(CASE WHEN rs.mode IN ('off') THEN COALESCE(rs.runtime_seconds, 0) ELSE 0 END)::INT AS runtime_seconds_mode_off,
        SUM(CASE WHEN rs.mode IN ('away', 'vacation') THEN COALESCE(rs.runtime_seconds, 0) ELSE 0 END)::INT AS runtime_seconds_mode_away,
        SUM(CASE WHEN rs.mode IN ('eco', 'energy_saver') THEN COALESCE(rs.runtime_seconds, 0) ELSE 0 END)::INT AS runtime_seconds_mode_eco,
        SUM(CASE WHEN rs.mode IN ('fan', 'auxheat', 'em_heat') OR (rs.mode NOT IN ('heat', 'cool', 'auto', 'heat-cool', 'off', 'away', 'vacation', 'eco', 'energy_saver') AND rs.mode IS NOT NULL) THEN COALESCE(rs.runtime_seconds, 0) ELSE 0 END)::INT AS runtime_seconds_mode_other,
